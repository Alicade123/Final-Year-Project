...
export default function ProductModal({
  ...
  const handlePrint = useReactToPrint({
    content: () => receiptRef.current,
    documentTitle: `Receipt_${newProduct?.lot_code || "Product"}`,
    onBeforeGetContent: () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const savedProduct = result?.data?.lot || formData;
      const farmer = farmers.find((f) => f.id === savedProduct.farmerId);
  ...
  const handleDownloadPDF = async () => {
    ...
    pdf.save(`ProductReceipt_${newProduct?.lot_code || Date.now()}.pdf`);
  };
          ...
          {success ? (
            <div className="text-center py-8">
              ...
              <div className="flex justify-center gap-3">
                <button
                  onClick={handlePrint}
                  className="bg-emerald-600 text-white px-6 py-3 rounded-xl shadow hover:bg-emerald-700 transition-all"
                >
                  🧾 Print Receipt
                </button>
                <button
                  onClick={handleDownloadPDF}
                  className="bg-teal-600 text-white px-6 py-3 rounded-xl shadow hover:bg-teal-700 transition-all"
                >
                  📥 Download PDF
                </button>
                <button
                  onClick={handleClose}
                  className="bg-gray-200 text-gray-800 px-6 py-3 rounded-xl hover:bg-gray-300 transition-all"
                >
                  Close
                </button>
              </div>

              {/* Hidden printable receipt - FIXED */}
              {newProduct && farmerDetails && (
