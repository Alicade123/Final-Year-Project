...
import AdminUsers from "../components/admin/AdminUsers";
import AdminHubs from "../components/admin/AdminHubs";
import AdminContacts from "../components/admin/AdminContacts"; // ⬅️ new
import { LogOut, Users, MapPin, MessageSquare } from "lucide-react";
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <nav className="flex flex-wrap gap-3 bg-white border-b px-6 py-3">
        {[
          ...
          { key: "hubs", icon: MapPin, label: "Hubs" },
          { key: "contacts", icon: MessageSquare, label: "Contacts" },
      ...
      <main className="flex-1 overflow-y-auto p-6">
        {tab === "users" && <AdminUsers />}
        {tab === "hubs" && <AdminHubs />}
        {tab === "contacts" && <AdminContacts />} {/* 👈 new tab */}
      </main>
    </div>
  );
}
